...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("""
            select p from Product p inner join Category c ON c.id = p.category.id\
    ...
    @Query(value = """
            ...
            from products p where p.is_activated = true and p.is_deleted = false order by rand() limit 9\
            """, nativeQuery = true)
    ...
    @Query(value = """
            select \
            p.product_id, p.name, p.description, p.current_quantity, p.cost_price, p.category_id, p.sale_price, p.image, p.is_activated, p.is_deleted \
            from products p where p.is_deleted = false and p.is_activated = true order by p.cost_price asc limit 9\
            """, nativeQuery = true)
